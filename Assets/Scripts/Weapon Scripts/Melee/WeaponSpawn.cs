...
public class WeaponSpawn : NetworkBehaviour {
    ...
    private IEnumerator WaitForCurrentAnim() {
        yield return new WaitForSeconds(playerAnimator.GetCurrentAnimatorStateInfo(1).normalizedTime * 0.3f);
        if (!notRigid) {
    ...
    private void CmdFire(Vector3 rigidbodyVelocity, float launchForce, Vector3 forward, Vector3 position, Quaternion rotation) {
        if (!notRigid) {
            ...
            Collider weaponInstance = Instantiate(colliderWeapon, position, rotation) as Collider;
